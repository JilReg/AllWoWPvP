...
function handleCheckboxChange(
  ...
  if (clickedCheckbox.value === "SETTINGS_CATEGORIES") {
    ...
    if (checkboxSettingsCategories.checked) {
      // Remove "hidden" class from all cardCategoryTag elements
      categoryTags.forEach((tag) => tag.classList.remove("hidden"));
    } else {
      // Add "hidden" class to all cardCategoryTag elements
  ...
  if (clickedCheckbox.value !== "SETTINGS_CATEGORIES" && window.location.pathname.endsWith("learn.html")) {
...
function loadFiltersFromLocalStorage() {
  ...
  if (savedFilters) {
    ...
    // Default: All filters are selected except SETTINGS_CATEGORIES
    selectedFilters = Array.from(checkboxes)
      .map((checkbox) => checkbox.value)
      .filter((value) => value !== "SETTINGS_CATEGORIES");
